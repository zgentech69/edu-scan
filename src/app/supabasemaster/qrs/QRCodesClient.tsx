...
const DIVISIONS = [
  'FY', 
  'SE-AIML', 'SE-CHEM', 'SE-COMP', 'SE-EXTC', 'SE-MECH', 'SE-CIVIL', 
  'TE-AIML', 'TE-CHEM', 'TE-COMP', 'TE-EXTC', 'TE-MECH', 'TE-CIVIL',
  'BE-AIML', 'BE-CHEM', 'BE-COMP', 'BE-EXTC', 'BE-MECH', 'BE-CIVIL'
];

export default function QRCodesClient({ allowedDivision }: { allowedDivision?: string }) {
  const [qrs, setQrs] = useState<Record<string, string>>({});
  const [printingDiv, setPrintingDiv] = useState<string | null>(null);
  
  const { searchQuery } = useAdminSearch();

  // If HOD, only allow their division
  const activeDivisions = allowedDivision 
    ? DIVISIONS.filter(d => d === allowedDivision)
    : DIVISIONS;
  ...
  const filteredDivisions = activeDivisions.filter(div => {
    const searchTarget = div === 'FY' ? 'first year' : (div.startsWith('SE-') || div.startsWith('TE-') || div.startsWith('BE-')) ? div.replace('-', ' ').toLowerCase() : `division ${div}`.toLowerCase();
    ...
    <div className="space-y-8 print:space-y-0 print:m-0 print:p-0">
        ...
        {filteredDivisions.map(div => (
          <div key={div} className="bg-sand-100 shadow-neu-flat rounded-3xl p-8 flex flex-col items-center border border-white/40 relative">
            <h3 className="text-3xl font-display font-bold text-sand-900 mb-6 text-center">
              {div === 'FY' ? 'First Year' : (div.startsWith('SE-') || div.startsWith('TE-') || div.startsWith('BE-')) ? div.replace('-', ' ') : `Division ${div}`}
            </h3>
            {qrs[div] ? (
            ...
            <p className="mt-6 text-sand-900/60 font-medium text-center">
              Scan to access subjects for {div === 'FY' ? 'First Year' : (div.startsWith('SE-') || div.startsWith('TE-') || div.startsWith('BE-')) ? div.replace('-', ' ') : `Division ${div}`}
            </p>
            ...
            <button 
              ...
              title={`Print ${div === 'FY' ? 'First Year' : (div.startsWith('SE-') || div.startsWith('TE-') || div.startsWith('BE-')) ? div.replace('-', ' ') : `Division ${div}`} QR`}
            >
              <Printer size={22} />
            </button>
          </div>
      ...
      <div className="fixed -left-[9999px] print:static print:block w-full bg-[#F6F4EB]">
        {activeDivisions.map((div, index) => {
          ...
          const isLastToPrint = printingDiv === 'all' ? index === activeDivisions.length - 1 : true;
          ...
          return (
            <div key={`print-${div}`} className={`print:flex print:flex-col print:items-center print:justify-between w-[210mm] h-[290mm] p-8 pt-10 bg-[#F6F4EB] mx-auto relative overflow-hidden box-border font-sans text-ink ${!isLastToPrint ? 'break-after-page' : ''}`}>
              ...
              <div className="absolute top-8 left-8 w-28 h-28 z-30 flex items-center justify-center overflow-hidden">
                <img src="/gitm-logo.png" alt="GITM Logo" className="w-[105%] h-[105%] object-contain [clip-path:inset(3px)]" />
              </div>
