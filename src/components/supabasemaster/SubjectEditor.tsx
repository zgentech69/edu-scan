      ...
      <div className="flex justify-between items-start mb-4">
        <div className="w-full">
          ...
          <div className="inline-block mt-2 px-2 py-1 bg-sand-200/50 rounded text-xs font-bold text-sand-900/60">
            Subject Mode: Multiple Teachers
          </div>
        </div>
      </div>
      ...
      {error && (
        ...
        <div className="mt-6 border-t border-sand-200/50 pt-4">
          ...
          <div className="space-y-3 mb-4">
            {subject.drive_links?.map(link => {
              // Only show links that have a teacher name (the new system)
              if (!link.teacher_name) return null;
              return (
            ...
            {(!subject.drive_links || subject.drive_links.filter(l => l.teacher_name).length === 0) && (
